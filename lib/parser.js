...
function parseTags(text) {
    var tagsParser = new nearley_1.default.Parser(nearley_1.default.Grammar.fromCompiled(hashtags_1.default));
    tagsParser.feed(text);
    var results = tagsParser.results.reduce(function (acc, _a) {
        var _b;
        var type = _a.type, value = _a.value;
        return (__assign(__assign({}, acc), (_b = {}, _b[type] = removeDuplicates(__spreadArray(__spreadArray([], __read((acc[type] || []))), [value])).filter(validHash), _b)));
    }, {});
    return results;
}
...
function parseLine(text, index) {
    var lineParser = new nearley_1.default.Parser(nearley_1.default.Grammar.fromCompiled(linetype_1.default));
    lineParser.feed(text);
    var line = lineParser.results[0];
    return __assign({ startIndex: index, endIndex: index, tags: parseTags(text) }, line);
}
exports.parseLine = parseLine;
function getLastIndex(lines, startIndex, parentSpaces) {
    if (parentSpaces === void 0) { parentSpaces = 0; }
    var i = startIndex;
...
function mergeText(source, texts, positions) {
    if (texts.length !== positions.length && texts.length === 0) {
        throw new Error("Wrong set of input for merging text, all should be of same size and greater than zero");
    }
    return __spreadArray([
        [0, positions[0] + 1]
    ], __read(positions.map(function (start, index, arr) {
...
function getUpdatedText(sourceFile, targetFile) {
    ...
    var temp = blocks.filter(function (block) { return block.tags.includeTag && block.tags.includeTag.length > 0; });
    var positions = temp.map(function (_a) {
        var startIndex = _a.startIndex;
        return startIndex;
    });
    var texts = temp.map(function (block) {
        return getBlocksForTag(targetFile.split("\n"), blocks2, block.tags.includeTag[0]).join("\n");
    });
    return mergeText(sourceFile.split("\n"), texts, positions);
}
exports.getUpdatedText = getUpdatedText;
function getTargetBlocksType(text) {
    var targetParser = new nearley_1.default.Parser(nearley_1.default.Grammar.fromCompiled(target_1.default));
...
function getTargetBlocks(texts) {
    var types = texts.map(function (text, index) { return (__assign(__assign({}, getTargetBlocksType(text)), { index: index })); });
    var pairs = [];
